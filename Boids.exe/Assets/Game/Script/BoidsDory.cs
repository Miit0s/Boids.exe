...
public class BoidsDory : MonoBehaviour
{
    ...
    public List<BoidsDory> newPack = new List<BoidsDory>(); //A changer
    bool finTemps;
    bool uneFois = false;
    ...
    void Update()
    {
        ...
        if (finTemps == false)
        {
            foreach (BoidsDory otherBoid in BoidsManagerDory.sharedInstanceDory.roBoids) //A changer
            {
        ...
        else
        {
            if (uneFois == true)
            {
                foreach (BoidsDory Boids in BoidsManagerDory.sharedInstanceDory.roBoids) //A changer
                {
            ...
            foreach (BoidsDory otherBoid in newPack) //A changer
            {
                Vector3 vecToOtherBoid = otherBoid.transform.position - transform.position;
    ...
    private void ChooseTarget(List<BoidsDory> boids) //A changer
    {
