...
public class BoidsCarp : MonoBehaviour
{
    ...
    public List<BoidsCarp> newPack = new List<BoidsCarp>(); //A changer
    bool finTemps;
    bool uneFois = false;
    ...
    void Update()
    {
        ...
        if (finTemps == false)
        {
            foreach (BoidsCarp otherBoid in BoidsManagerCarp.sharedInstanceCarp.roBoids) //A changer
            {
        ...
        else
        {
            if (uneFois == true)
            {
                foreach (BoidsCarp Boids in BoidsManagerCarp.sharedInstanceCarp.roBoids) //A changer
                {
            ...
            foreach (BoidsCarp otherBoid in newPack) //A changer
            {
                Vector3 vecToOtherBoid = otherBoid.transform.position - transform.position;
    ...
    private void ChooseTarget(List<BoidsCarp> boids) //A changer
    {
